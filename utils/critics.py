...
class Attn_Coop_Critic(nn.Module):
    """
    Attention network, used as critic for all agents. Each agent gets its own
    observation and action, and can also attend over the other agents' encoded
    observations and actions.
    ...
    def __init__(self, sa_sizes, hidden_dim=32, norm_in=True, attend_heads=1):
        ...
        Inputs:
            ...
            attend_heads (int): Number of attention heads to use (use a number
                                that hidden_dim is divisible by)
        """
        super(Attn_Coop_Critic, self).__init__()
        assert (hidden_dim % attend_heads) == 0
        self.sa_sizes = sa_sizes
        self.nagents = len(sa_sizes)
        self.attend_heads = attend_heads
        ...
        for sdim, adim in sa_sizes:
            ...
            # Critic head for extrinsic reward
            ext_critic = nn.Sequential()
            ext_critic.add_module('ext_critic_fc1', nn.Linear(2 * hidden_dim,
                                                              hidden_dim))
            ext_critic.add_module('ext_critic_nl', nn.LeakyReLU())
            ext_critic.add_module('ext_critic_fc2', nn.Linear(hidden_dim,
                                                              odim))
            self.ext_critics.append(ext_critic)

            # Critic head for intrinsic reward
            # T.B.U. (It also could be shared among agents)
            int_critic = nn.Sequential()
            int_critic.add_module('int_critic_fc1', nn.Linear(2 * hidden_dim,
                                                              hidden_dim))
            int_critic.add_module('int_critic_nl', nn.LeakyReLU())
            int_critic.add_module('int_critic_fc2', nn.Linear(hidden_dim,
                                                              odim))
            self.int_critics.append(ext_critic)

            # Critic head for interaction reward
            # T.B.U.
